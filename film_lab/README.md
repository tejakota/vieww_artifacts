...
Produced by autonomous sessions (2026-09-24/25) running the create → render → VLM-audit → iterate loop against the actual vieww framework. **Thirty-six element experiments** ("props" for the film), each rendered with per-run receipts — round 3 covered every scene-graph effect, round 4 demonstrated the todo-upgrades doors in use, round 5 closed the two remaining framework notes (U-06's guard, U-20's counter), caught U-22 with the new census and added the pixel probe, and **round 6 is the berserk spectrum: eleven new plates, each one hitting a different tolerance axis of the framework — the shape-count record (60,527/frame), the rect ceiling (57,602/frame), 4K master resolution, glyph-run storms, recursion, procedural cities, and the three genesis arcs the author named (line → 3D avatar, text → blurred fade-away, drop → entire ocean)**.
