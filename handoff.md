...
- Lab artifact set (film_lab, round 6 onward): every plate ships
  **anim.gif + sheet.png + metrics.txt**. The GIF is the human-facing
  motion receipt — it loops inline on GitHub with no codec; the sheet
  stays the audit surface (motion still never reaches the assistant);
  the metrics carry the measured `gif=` line (dimensions, fps, bytes).
  Generated by the harness itself (`film_lib::anim_gif`) or batch from
  rendered frames (`film_lab/tools/make_gifs.sh`).
