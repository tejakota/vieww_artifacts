...
/// calls renders as empty, not as a hint. `SceneReport::open_subpath_fills`
/// (vieww-paint) counts them (todo-upgrades U-20).
///
...
impl Path {
    ...
    pub fn move_to(&mut self, point: Offset) -> &mut Self {
        debug_assert!(
            point.dx.is_finite() && point.dy.is_finite(),
            "move_to({point:?}): a NaN or infinite coordinate poisons the \
             geometry into a plausible shape nobody drew (NaN) or drops the \
             whole path (infinity) — usually a normalise that divided by a \
             zero-length vector, or a perspective divide by a Z on the camera \
             plane (todo-upgrades U-16)"
        );
    ...
    pub fn line_to(&mut self, point: Offset) -> &mut Self {
        debug_assert!(
            point.dx.is_finite() && point.dy.is_finite(),
            "line_to({point:?}): non-finite coordinates corrupt the geometry \
             silently (todo-upgrades U-16)"
        );
