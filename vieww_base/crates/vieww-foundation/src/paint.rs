...
impl Gradient {
    ...
    /// # The seam
    ///
    /// A full-turn sweep's first and last angle are the same ray, so unless
    /// the ramp's first and last colour are identical there is a hard seam
    /// through the shape. Build the ramp with
    /// [`with_stops_mirrored`](Self::with_stops_mirrored) and the wrap is
    /// continuous by construction (todo-upgrades U-10).
    #[must_use]
    ...
    ///
    /// That seam is real: unless the ramp is a palindrome the wrap lands a
    /// vertical discontinuity straight down the middle of the fill — see
    /// [`sweep`](Self::sweep) and [`with_stops_mirrored`](Self::with_stops_mirrored).
    #[must_use]
    ...
    ///
    /// Stops are kept in the order given, not sorted: a caller listing them
    /// out of order has made a mistake worth seeing rather than one worth
    /// silently repairing. Out of order, the ramp renders flat from the first
    /// colour — see [`with_stops_mirrored`](Self::with_stops_mirrored) for the
    /// sweep case, and **an animated phase must re-sort**: a ramp whose offsets
    /// wrap through `1.0` as the phase advances is out of order for exactly one
    /// frame per stop per cycle, which is the hardest class of bug to see in a
    ...
    pub fn with_stops(mut self, stops: &[(f32, Color)]) -> Self {
        debug_assert!(
            ...
            "gradient stops must be non-decreasing: {stops:?} — a ramp listed \
             out of order renders flat from its first colour, and an animated \
             phase that wraps offsets must re-sort"
        );
        self.count = 0;
    ...
    /// Replace the stops with a **palindrome**: the given half-ramp, then its
    /// mirror image, so the first and last colour are the same by construction
    /// (todo-upgrades U-10).
    ///
    /// # Why a sweep ramp must be a palindrome
    ///
    /// A sweep gradient's start and end angles are **the same ray**. Unless
    /// the first and last stop carry the same colour, the ramp has a
    /// discontinuity there — and because [`Gradient::conic()`](Self::conic)
    /// starts at twelve o'clock, the artifact is a hard vertical line straight
    /// down the middle of whatever you filled. It is not a bug; it is the only
    /// thing a sweep gradient can do with an open ramp — but nothing used to
    /// say so, and the failure looks exactly like a rasterizer bug.
    ///
    /// # The mapping
    ///
    /// Give the stops of the **first half** of the ramp; the mirror lands at
    /// `2·t − o` where `t` is the last stop's offset, so a half-ramp ending at
    /// `0.5` fills the full turn and one ending earlier runs flat (in the
    /// mirrored first colour) for the remainder. `stops[0]` is the wrap
    /// colour: it appears at both ends of the finished ramp.
    ///
    /// ```
    /// use vieww_foundation::{Color, Gradient};
    ///
    /// // Chrome: violet to white and back — no seam at twelve o'clock.
    /// let g = Gradient::conic().with_stops_mirrored(&[
    ///     (0.0, Color::rgb(139, 92, 246)),
    ///     (0.5, Color::WHITE),
    /// ]);
    /// let stops = g.stops();
    /// assert_eq!(stops.first().map(|s| s.color), stops.last().map(|s| s.color));
    /// ```
    #[must_use]
    pub fn with_stops_mirrored(self, stops: &[(f32, Color)]) -> Self {
        let Some(&(tip, _)) = stops.last() else {
            return self.with_stops(stops);
        };
        let mut ramp: Vec<(f32, Color)> = stops.to_vec();
        for &(offset, color) in stops.iter().rev().skip(1) {
            let mirrored = 2.0 * tip - offset;
            // Non-decreasing by construction (offset < tip ⇒ mirrored > tip),
            // but guard the degenerate case where a stop sits exactly at the
            // tip and would duplicate it.
            if ramp.last().is_none_or(|&(last, _)| mirrored > last + 1e-6) {
                ramp.push((mirrored, color));
            }
        }
