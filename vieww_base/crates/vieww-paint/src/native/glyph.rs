...
// -------------------------------------------------------------- public door
//
// See each function's doc; both exist because of todo-upgrades U-03 — the
// framework could draw text and could not hand you its shape.

/// The outline of one glyph as a [`Path`], in **font units**, Y already
/// flipped to the screen convention — the public door (todo-upgrades U-03).
///
/// The outlines the rasterizer fills have always been extracted here and
/// were `pub(crate)`, so every caller that wanted a stroke-on title, a
/// letterform to morph, or a wordmark to scatter into the frames it was
/// rendered from re-parsed the font with `ttf-parser` one layer above where
/// this already does it. This function is that layer, opened.
///
/// Combine with a shaped run the way the rasterizer does: scale by
/// `size / units_per_em` (see [`units_per_em`]), then translate by each
/// glyph's offset — the same arithmetic `place_glyph` applies before
/// flattening. `None` means the face has no outline for that glyph — a
/// missing glyph, or a bitmap/COLR-only one — which is the same contract the
/// internal cache reports.
///
/// Uncached and re-parsing on every call: this is for geometry, not per-frame
/// rasterization, and the internal cache stays keyed by the render pipeline's
/// own `(font id, glyph id)` space (see the module docs for why that split is
/// safe — an outline is derived, read-only data).
#[must_use]
pub fn outline_glyph(
    ...
    extract_outline(face_bytes, face_index, glyph_id, variations)
}

/// A face's units-per-em — the divisor that turns font units into pixels at
/// a given size. A 12 px run of a 2048-upem face scales outline coordinates
/// by `12.0 / 2048.0` (todo-upgrades U-03, the door's second half).
///
/// `None` when the bytes do not parse as a face at `face_index`, so a caller
/// can distinguish "unparseable font" from "no outline for this glyph" — two
/// failures that both come back as `None` from [`outline_glyph`] and mean
/// completely different things. A face that parses reports its real upem;
/// nothing is assumed.
#[must_use]
pub fn units_per_em(face_bytes: &[u8], face_index: u32) -> Option<f32> {
