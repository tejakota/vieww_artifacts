...
use vieww_foundation::{
    BlendMode, Color, Gradient, GradientStop, Image, ImageFilter, Offset, Path, Rect, Shadow,
    StrokeCap, StrokeJoin, StrokeStyle, TextStyle,
};
...
/// todo-upgrades U-15, at the renderer rather than the kernel: a blurred
/// layer whose ink runs past every edge of the surface — a background, a
/// frame-wide glow, the "field with no edge anywhere inside the frame" the
/// upgrade note measured — must stay solid to the frame's corners.
///
/// The layer's offscreen buffer is intersected with the surface, so full-bleed
/// ink lands against the buffer's own edges; the blur used to read past those
/// as transparent black, which measured 100/255 at the frame's edges and
/// 39/255 at its corners. Edge-clamped sampling makes a uniform field a fixed
/// point of the blur instead — asserted on the real composite over black, so
/// the whole acquire/blur/composite path has to agree, not just the kernel.
#[test]
fn a_full_bleed_blurred_layer_stays_solid_to_the_frame_corners() {
    for sigma in [4.0, 8.0, 24.0] {
        // The fill extends past all four sides of the 96×96 surface.
        let ink = Rect::new(-24.0, -24.0, 120.0, 120.0);
        let mut scene = Scene::new();
        scene.push_filtered_layer(ink, 1.0, BlendMode::Normal, ImageFilter::blur(sigma));
        scene.fill_path(&Path::rect(ink), Color::WHITE.into());
        scene.pop_layer();
        let (pixels, report) = render("full_bleed_blur", &scene, 96, 96, Color::BLACK);
        assert_eq!(report.filtered_layers, 1, "sigma {sigma}: the blur ran");
        ...
        for (x, y) in [
            ...
            assert_eq!(
                (p.r, p.g, p.b, p.a),
                (255, 255, 255, 255),
                "sigma {sigma}: a full-bleed layer faded at ({x}, {y})"
            );
        }
    }
}

/// todo-upgrades U-14, at the renderer: a motion blur smears along its ray
/// and not across it. A small bright dot inside a directionally-blurred layer
/// over black: the pixel four steps **along** the ray lights up, the pixel
/// four steps **across** it stays dark — the property a shatter plate's
/// shards depend on, and the one that separates this from an isotropic blur.
#[test]
fn a_directional_blur_smears_along_its_ray_through_the_renderer() {
    // The dot, well inside the layer's reach-grown bounds.
    let dot = Rect::new(46.0, 46.0, 50.0, 50.0);
    let layer = Rect::new(12.0, 12.0, 84.0, 84.0);
    let mut scene = Scene::new();
    scene.push_filtered_layer(
        layer,
        1.0,
        BlendMode::Normal,
        ImageFilter::directional_blur(3.0, 0.0),
    );
    scene.fill_path(&Path::rect(dot), Color::WHITE.into());
    scene.pop_layer();
    let (pixels, report) = render("directional_blur", &scene, 96, 96, Color::BLACK);
    assert_eq!(report.filtered_layers, 1, "the directional blur ran");

    // Ink shows in the colour channels over the opaque black base — the
    // composite's alpha is 255 everywhere and says nothing. Three radius-3
    // passes along the ray reach about ±9 px: 4 px past the dot's right edge
    // must be lit, and 7 px below its bottom edge — further than the dot's
    // own anti-aliasing but straight *across* a ray with no vertical extent —
    // must still be black.
    let along = pixels.pixel(54, 48);
    assert!(
        along.r > 20,
        "four steps along a horizontal ray at sigma 3 must carry ink: {along:?}"
    );
    let across = pixels.pixel(48, 57);
    assert!(
        across.r < 5,
        "seven steps across the ray must stay black: {across:?}"
    );
}
