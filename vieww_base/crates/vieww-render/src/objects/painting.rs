...
fn replay(canvas: &mut dyn Canvas, item: &Sketch) {
    ...
    match item {
        ...
        Sketch::Layer {
            ...
            //
            // `blend` rides along to both layer pushes (todo-upgrades U-01):
            // `Canvas::push_layer` and `push_filtered_layer` have taken a
            // `BlendMode` since the beginning; this replay used to hard-code
            // `Normal`, which made the mode unreachable from a sketch.
            ...
            if *blur > 0.0 {
                ...
                canvas.push_layer(bounds, *alpha, *blend);
            }
