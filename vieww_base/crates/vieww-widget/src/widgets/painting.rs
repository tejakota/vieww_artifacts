...
pub trait Painter: fmt::Debug + 'static {
    ...
    ///
    /// The recording happens before anything can look at its alpha: a painter
    /// that builds 100,000 items and hides them under a layer at `alpha: 0.0`
    /// still pays for all 100,000 — `Sketch::is_invisible` is checked at
    /// *replay*, by which time the allocation has happened (todo-upgrades
    /// U-19: 41.8 ms for a frame that rasterizes one shape). Skip the work in
    /// `paint`, not with `alpha`; this is the property of recording into a
    /// value rather than issuing calls, and it is the same property that pays
    /// for damage tracking everywhere else.
    fn paint(&self, book: &mut Sketchbook, size: Size);
