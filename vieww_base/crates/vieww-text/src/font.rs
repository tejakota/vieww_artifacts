...
/// Load the embedded fallback faces into `db`, asserting that at least one
/// of them actually parsed (todo-upgrades U-09).
///
/// A `.ttf` that fails to parse used to enter `fontdb` as a silent
/// zero-face contribution: the bytes were accepted, the shaper found no
/// face, and **every glyph in every render came back as nothing** — a blank
/// page with four coloured squares on it and no error anywhere. The bench
/// that found it was running a tree whose font assets were placeholder
/// notes standing where the binaries should have been, which is exactly the
/// kind of corruption an assert can name. Parsing each face before it is
/// registered converts the blank screen into a panic that names the files.
fn register_embedded(db: &mut fontdb::Database) {
    let named: [(&str, &[u8]); 6] = [
        ("assets/DejaVuSans-subset.ttf", EMBEDDED_FONT),
        ("assets/DejaVuSans-Bold-subset.ttf", EMBEDDED_FONT_BOLD),
        ("assets/DejaVuSans-Oblique-subset.ttf", EMBEDDED_FONT_ITALIC),
        ("assets/DejaVuSansMono-subset.ttf", EMBEDDED_MONO),
        ("assets/DejaVuSansMono-Bold-subset.ttf", EMBEDDED_MONO_BOLD),
        ("assets/NotoSansCJK-subset.ttf", EMBEDDED_CJK),
    ];
    let parsed: Vec<&str> = named
        .iter()
        .filter(|(_, bytes)| ttf_parser::Face::parse(bytes, 0).is_ok())
        .map(|(name, _)| *name)
        .collect();
    assert!(
        !parsed.is_empty(),
        "no embedded face parsed — crates/vieww-text's font assets are corrupt \
         or placeholder notes (todo-upgrades U-09). Parsed {parsed:?} of \
         {} candidates; restore the real binaries. The alternative this \
         assert replaces is a render with no glyphs and no error.",
        named.len()
    );
...
impl FontStore {
    ...
    pub fn new() -> Self {
        ...
        register_embedded(system.db_mut());
        Self {
            system,
            data: HashMap::new(),
