...
# download behind when an install is abandoned. `ci/android-env.sh` already
...
# shellcheck source=ci/android-env.sh
. "$root/ci/android-env.sh"
...
# `crates/android/Cargo.toml`'s `min_sdk_version`, which matches vieww's own —
# and what CI links against. Linking to a lower API than the device runs is the supported
