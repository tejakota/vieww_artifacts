...
#
# It was copied, and the copy cost a session. `ci/device-suite.sh` already knew
...
# `ndk/30.0.15729638` on this machine is exactly that. `ci/reload-guest-android.sh`
...
android_env() {
    ...
    # file exists: `ci/reload-android.sh` was written without it and failed on
